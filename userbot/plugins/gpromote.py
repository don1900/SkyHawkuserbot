...
async def get_full_user(event):  
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("Error... Please report at @SkyhawkSupport", str(err))           
...
async def gben(userbot):
    ...
    try:
        ...
        pass
    if me == user:
       k = await Skyhawk.edit("U want to promote urself waah.. Chutiye")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    ...
    if user:
        ...
        for x in telchanel:
          try:
             ...
             i += 1
             await Skyhawk.edit(f"**Promoted in Chats **: `{i}`")
          except:
             pass
    else:
        await Skyhawk.edit(f"**Reply to a user you dumbo !!**")
    return await Skyhawk.edit(
...
async def gben(userbot):
    ...
    try:
        ...
        pass
    if me == user:
       k = await Skyhawk.edit("U want to demote urself waah.. Chutiye")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    ...
    if user:
        ...
        for x in telchanel:
          try:
             await userbot.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
             await Skyhawk.edit(f"**Demoted in Chats **: `{i}`")
          except:
             pass
    else:
        await Skyhawk.edit(f"**Reply to a user you dumbo !!**")
