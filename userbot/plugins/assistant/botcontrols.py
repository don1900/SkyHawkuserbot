...
from userbot import SkyHawkbot
    ...
    for user in get_all_starters():
        try:
            await event.client.send_message(
                int(user.user_id), "Succesfully Sent You a **new** Broadcast."
            )
            ...
            if count % 5 == 0:
                try:
                    prog_ = (
                        ...
                        + f"\n\n• ✔️ **Successfully Sent Your Broadcast Messages To ** :  `{count}` users.\n"
                        + f"• ✖️ **Failed Sending Your Broadcast Messgae To ** :  `{len(blocked_users)}`users."
                    )
