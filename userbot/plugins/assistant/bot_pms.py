...
from userbot import Config, SkyHawkbot
...
@SkyHawkbot.bot_cmd(pattern="start ?(/*))",incoming=True,func=lambda e: e.is_private,)
async def bot_start(event):
    ...
    if chat.id != Config.OWNER_ID:
        start_msg = f"Hello, {firstname} ! Nice To Meet You, Well I Am {bot_id}, An Powerfull Assistant Bot. \n\nMy [➤ Master](tg://user?id={bot.uid}) \nYou Can Talk/Contact My Master Using This Bot. \n\nIf You Want Your Own Assistant You Can Deploy From Button Below. \n\nPowered By [SkyHawk](https://t.me/SkyHawkSupport)"
        buttons = [
            (
                Button.url("Repo", "https://github.com/don1900/SkyHawk"),
                Button.url(
                    "Deploy",
                    "https://dashboard.heroku.com/new?button-url=https%3A%2F%2Fgithub.com%2Fdon1900%2Fskyhawk&template=https%3A%2F%2Fgithub.com%2Fdon1900%2Fskyhawk",
                ),
            )
        ]
    else:
        start_msg = f"Hi Master, It's Me {bot_id}, Your Assistant ! \nWhat You Wanna Do today ?"
        buttons = None
    try:
...
@Skyhawkbot.bot_cmd(edited=True)
...
async def send_flood_alert(user_) -> None:
    ...
    if found:
        ...
        try:
            ...
            if BOTLOG:
                await SkyHawkbot.tgbot.send_message(BOTLOG_CHATID, "**Unblock your bot !**")
    if FloodConfig.ALERT[user_.id].get("fa_id") is None and fa_msg:
        FloodConfig.ALERT[user_.id]["fa_id"] = fa_msg.id


@SkyHawkbot.tgbot.on(CallbackQuery(data=re.compile(b"bot_pm_ban_([0-9]+)")))
@check_owner
...
@SkyHawkbot.tgbot.on(CallbackQuery(data=re.compile(b"toggle_bot-antiflood_off$")))
@check_owner
...
@SkyHawkbot.bot_cmd(incoming=True, func=lambda e: e.is_private)
@SkyHawkbot.bot_cmd(edited=True, func=lambda e: e.is_private)
