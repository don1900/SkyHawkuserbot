...
from SkyhawkBot import CmdHelp, bot as SkyhawkBot, ALIVE_NAME
from SkyhawkBot.utils import admin_cmd, sudo_cmd, edit_or_reply as eor

DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "Your Lover"

aura = SkyhawkBot.uid

Skyhawk = f"[{DEFAULTUSER}](tg://user?id={aura})"

@SkyhawkBot.on(admin_cmd(pattern="hvd$"))
@SkyhawkBot.on(sudo_cmd(pattern="hvd$", allow_sudo=True))
...
@SkyhawkBot.on(admin_cmd(pattern="vday$"))
@SkyhawkBot.on(sudo_cmd(pattern="vday$", allow_sudo=True))
async def gif(event):
    if event.fwd_from:
        return
