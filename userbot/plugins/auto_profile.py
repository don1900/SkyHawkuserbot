...
DEFAULTUSERBIO = str(BIO_MSG) if BIO_MSG else "Legendary SkyhawkBot"
DEL_TIME_OUT = 60
...
async def _(event):
    if event.fwd_from:
        return
    Skyhawk = await edit_or_reply(event, "`Starting AutoName Please Wait`")
    if event.fwd_from:
        return
    ...
    await Skyhawk.edit(f"Auto Name has been started my Master")
