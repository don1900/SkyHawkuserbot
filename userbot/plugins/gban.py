...
Harsh = str(ALIVE_NAME) if ALIVE_NAME else "Skyhawk User"
papa = borg.uid
...
async def gban(userbot):
    ...
    if not sender.id == hum.id:
        SkyhawkBot = await edit_or_reply(ids, "Trying to gban this retard!")
    else:
        SkyhawkBot = await edit_or_reply(ids, "`Ok! Gbaning this piece of shit....`")
    hum = await userbot.client.get_me()
    await SkyhawkBot.edit(f"`🔥Global Ban Iz Cumin'💦.... Wait and watch nigga🚶`")
    ...
    try:
        ...
        return await SkyhawkBot.edit(f"**Something W3NT Wrong 🤔**")
    if user:
        if user.id == 1837224931 or user.id == 1869741341 or user.id == 1642269125:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await SkyhawkBot.edit(f"Gbaning This retard🚶\n\nTotal Chats :- `{a}`")
            except:
                b += 1
    else:
        await SkyhawkBot.edit(f"`Either reply to a user or gib me user id/name`")
    try:
        if gmute(user.id) is False:
            return await SkyhawkBot.edit(f"**Error! User already gbanned.**")
    except:
        pass
    return await SkyhawkBot.edit(
...
async def gunban(userbot):
    ...
    hum = await ids.client.get_me()
    if not sender.id == hum.id:
        SkyhawkBot = await edit_or_reply(ids, "`Trying to ungban this kid...`")
    else:
        SkyhawkBot = await edit_or_reply(ids, "`Ungban in progress...`")
    hum = await userbot.client.get_me()
    await SkyhawkBot.edit(f"`Trying to ungban this kiddo...`")
    my_mention = "[{}](tg://user?id={})".format(hum.first_name, hum.id)
    f"@{hum.username}" if hum.username else my_mention
    await userbot.get_chat()
    a = b = 0
    ...
    try:
        ...
        return await SkyhawkBot.edit("**Som3ting W3nt Wr0ng**")
    if user:
        if user.id == 1837224931 or user.id == 1869741341 or user.id == 1642269125:
            return await SkyhawkBot.edit("**You need to grow some balls to gban / ungban my creator**")
        try:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await SkyhawkBot.edit(f"Ok! Now Ungbaning this kiddo.\nChats:- `{a}`")
            except:
                b += 1
    else:
        await SkyhawkBot.edit("**Reply to a user**")
    try:
        if ungmute(user.id) is False:
            return await SkyhawkBot.edit("**Error! User already ungbanned.**")
    except:
        pass
    return await SkyhawkBot.edit(
