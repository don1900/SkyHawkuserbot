...
async def _(event):
    ...
    Skyhawkevent = await edit_or_reply(event, "`Executing.....`")
    process = await asyncio.create_subprocess_sSkyhawk(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
    result = str(stdout.decode().strip()) + str(stderr.decode().strip())
    Skyhawkuser = await event.client.get_me()
    if Skyhawkuser.username:
        curruser = Skyhawkuser.username
    else:
        curruser = "SkyhawkBot"
    uid = os.geteuid()
    if uid == 0:
...
async def _(event):
    ...
    Skyhawkevent = await edit_or_reply(event, "`Running ...`")
...
CmdHelp("evaluators").add_command(
  ...
  'exec', '<command>', 'Execute a Terminal command on SkyhawkBot server and shows details'
).add_command(
