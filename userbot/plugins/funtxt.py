...
async def hmm(Skyhawk):
    if Skyhawk.fwd_from:
        return
    reactcat = nekos.textcat()
    await edit_or_reply(Skyhawk, reactcat)
...
async def hmm(Skyhawk):
    if Skyhawk.fwd_from:
        return
    whySkyhawk = nekos.why()
    await edit_or_reply(Skyhawk, whySkyhawk)
...
async def hmm(Skyhawk):
    if Skyhawk.fwd_from:
        return
    factSkyhawk = nekos.fact()
    await edit_or_reply(Skyhawk, factSkyhawk)
