...
async def download(event):
    ...
    if Var.GIT_REPO_NAME is None:
        await edit_or_reply(event, "`Please ADD Proper Github Repo Name of SkyhawkBot`")
        return
    SkyhawkBot = await edit_or_reply(event, "Processing ...")
    ...
    try:
        ...
        await SkyhawkBot.edit(str(e))
    else:
        end = datetime.now()
        ms = (end - start).seconds
        await event.delete()
        await SkyhawkBot.edit(
            "Downloaded to `{}` in {} seconds.".format(downloaded_file_name, ms)
        )
        await SkyhawkBot.edit("Committing to Github....")
        await git_commit(downloaded_file_name, SkyhawkBot)


async def git_commit(file_name, SkyhawkBot):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await SkyhawkBot.edit("`File Already Exists`")
            create_file = False
    ...
    if create_file == True:
        ...
        try:
            ...
            await SkyhawkBot.edit("Cannot Upload Plugin")
    else:
        return await SkyhawkBot.edit("`Committed Suicide`")
