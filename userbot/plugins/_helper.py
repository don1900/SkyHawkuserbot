...
async def yardim(event):
    ...
    if tgbotusername is not None or Skyhawk_input == "text":
        results = await event.client.inline_query(tgbotusername, "@SkyhawkUserbot")
...
async def info(event):
    ...
    if input_str == "text":
        string = (
            "Total {count} commands found in {plugincount} sudo plugins of SkyhawkBot\n\n"
        )
        Skyhawkcount = 0
        plugincount = 0
        for i in sorted(SUDO_LIST):
            ...
            string += "\n"
        if len(string) > 4095:
            data = string.format(count=Skyhawkcount, plugincount=plugincount)
            key = (
                requests.post(
            ...
            reply_text = f"All commands of the SkyhawkBot are [here]({url})"
            await event.reply(reply_text, link_preview=False)
            return
        await event.reply(
            string.format(count=Skyhawkcount, plugincount=plugincount), link_preview=False
        )
        return
    if input_str:
        if input_str in SUDO_LIST:
            ...
            for i in SUDO_LIST[input_str]:
                ...
                Skyhawkcount += 1
            await event.reply(
                string.format(count=Skyhawkcount, input_str=input_str), parse_mode="HTML"
            )
        else:
        ...
        for i in sorted(SUDO_LIST):
            ...
            string += " "
            Skyhawkcount += 1
        await event.reply(string.format(count=Skyhawkcount), parse_mode="HTML")
