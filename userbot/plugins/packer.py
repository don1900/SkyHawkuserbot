...
@SkyhawkBot.on(admin_cmd(pattern=r"unpack", outgoing=True))
@SkyhawkBot.on(sudo_cmd(pattern=r"unpack"))
async def _(event):
...
@SkyhawkBot.on(admin_cmd(pattern="repack ?(.*)", outgoing=True))
@SkyhawkBot.on(sudo_cmd(pattern="repack ?(.*)", allow_sudo=True))
async def _(event):
