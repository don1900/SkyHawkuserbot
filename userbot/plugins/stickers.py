...
SkyhawkBot = Config.CUSTOM_STICKER_PACK_NAME
...
async def kang(args):
    ...
    if photo:
        ...
        packname = f"AB_{user.username}_SkyhawkBot_{pack}"
        packnick = (
            f"{SkyhawkBot} Vol.{pack}"
            if SkyhawkBot
            else f"@{user.username}'s SkyhawkBot Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
