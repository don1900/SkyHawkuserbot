...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    Skyhawkevent = await edit_or_reply(event, " `Scanning This media..... wait👀`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await Skyhawkevent.edit("`Please unblock `@DrWebBot `and try again`")
            return
...
async def _(event):
    if event.fwd_from:
        return
    Skyhawkevent = await edit_or_reply(event, "...")
    start = datetime.now()
    ...
    await Skyhawkevent.edit("Created BarCode in {} seconds🤓".format(ms))
...
async def _(event):
    if event.fwd_from:
        return
    Skyhawkevent = await edit_or_reply(event, "`processiong...........`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
Year: {}""".format(
        ...
        await Skyhawkevent.edit(output_str, link_preview=True)
    else:
        await Skyhawkevent.edit("xkcd n.{} not found!".format(xkcd_id))
...
CmdHelp("tools").add_command(
  ...
  "currency", "<amount> <from> <to>", "Currency converter for SkyhawkBot", ".currency 10 usd inr"
).add_command(
