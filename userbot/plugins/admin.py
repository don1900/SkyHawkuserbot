...
async def promote(promt):
    ...
    Skyhawkevent = await edit_or_reply(promt, "Promoting...")
    ...
    try:
        ...
        await Skyhawkevent.edit("Promoted Successfully!")
    except BadRequestError:
        await Skyhawkevent.edit(NO_PERM)
        return
    if BOTLOG:
...
async def demote(dmod):
    ...
    if not admin and not creator:
        ...
        return
    Skyhawkevent = await edit_or_reply(dmod, "Demoting...")
    rank = "??????"
    ...
    try:
        ...
        await Skyhawkevent.edit(NO_PERM)
        return
    await Skyhawkevent.edit("Demoted Successfully!")
    if BOTLOG:
...
async def ban(bon):
    ...
    if not user:
        return
    Skyhawkevent = await edit_or_reply(bon, "Banning this retard")
    try:
    ...
    try:
        ...
        await Skyhawkevent.edit("I ain't got msg deleting right. But still Banned!")
        return
    if reason:
        await Skyhawkevent.edit(f"{str(user.id)} is banned !!\nReason: {reason}")
    else:
        await Skyhawkevent.edit(f"{str(user.id)} is banned!")
    if BOTLOG:
...
async def nothanos(unbon):
    ...
    if not admin and not creator:
        ...
        return
    Skyhawkevent = await edit_or_reply(unbon, "Unbanning...")
    ...
    try:
        ...
        await Skyhawkevent.edit("Unbanned Successfully!")
        if BOTLOG:
        ...
        await Skyhawkevent.edit("Sorry I Can't Unban This Retard!")
...
async def kick(usr):
    ...
    if not user:
        ...
        return
    Skyhawkevent = await edit_or_reply(usr, "Kicking...")
    try:
        await usr.client.kick_participant(usr.chat_id, user.id)
        await sleep(0.5)
    except Exception as e:
        await Skyhawkevent.edit(NO_PERM + f"\n{str(e)}")
        return
    if reason:
        ...
        await Skyhawkevent.edit(f"Kicked [{user.first_name}](tg://user?id={user.id})!")
    if BOTLOG:
