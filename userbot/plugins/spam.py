...
async def bigspam(Skyhawk):
    if not Skyhawk.text[0].isalpha() and Skyhawk.text[0] not in ("/", "#", "@", "!"):
        Skyhawk_msg = Skyhawk.text
        SkyhawkBot_count = int(Skyhawk_msg[9:13])
        Skyhawk_spam = str(Skyhawk.text[13:])
        for i in range(1, SkyhawkBot_count):
            await Skyhawk.respond(Skyhawk_spam)
        await Skyhawk.delete()
        if LOGGER:
            await Skyhawk.client.send_message(
...
CmdHelp("spam").add_command(
  ...
  "bigspam", "<count> <text>", "Sends the text 'X' number of times. This what SkyhawkBot iz known for. The Best BigSpam Ever", ".bigspam 5000 Hello"
).add()
