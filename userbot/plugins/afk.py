...
from userbot import ALIVE_NAME, Skyhawkversion
from SkyhawkBot.utils import admin_cmd, edit_or_reply
...
async def set_not_afk(event):
    ...
    if ".afk" not in current_message and "yes" in USER_AFK:  # pylint:disable=E0602
        SkyhawkBot = await borg.send_message(
            event.chat_id,
            "🔥__Back alive!__\n**No Longer afk.**\n⏱️ `Was afk for:``"
            + total_afk_time
            + "`", file=Skyhawkpic
        ...
        try:
            ...
            await borg.send_message(  # pylint:disable=E0602
                ...
                + "Ask in @SkyhawkSupport to get help setting this value\n\n `{}`".format(str(e)),
                reply_to=event.message.id,
                silent=True,
            )
        await asyncio.sleep(5)
        await SkyhawkBot.delete()
...
async def on_afk(event):
  ...
  if reason
        ...
        msg = await event.reply(message_to_reply, file=Skyhawkpic)
...
async def _(event):
    ...
    Skyhawkpic = await event.client.download_media(aura)
    if not USER_AFK:  # pylint:disable=E0602
        ...
        USER_AFK = f"yes: {reason} {Skyhawkpic}"  # pylint:disable=E0602
        if reason:
            await borg.send_message(
                event.chat_id, f"__**I'm going afk🚶**__ \n⚜️ Because `{reason}`", file=Skyhawkpic
            )
        else:
            await borg.send_message(event.chat_id, f"**I am Going afk!**🚶", file=Skyhawkpic)
        await asyncio.sleep(0.001)
        await event.delete()
        try:
            await borg.send_message(  # pylint:disable=E0602
                Config.PRIVATE_GROUP_BOT_API_ID,  # pylint:disable=E0602
                f"#AFKTRUE \nSet AFK mode to True, and Reason is {reason}",file=Skyhawkpic
            )
        except Exception as e:  # pylint:disable=C0103,W0703
