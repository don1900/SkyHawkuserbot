...
from SkyhawkBot.utils import admin_cmd, edit_or_reply, progress, sudo_cmd
...
async def _(Skyhawk):
    if Skyhawk.fwd_from:
        return
    reply_to_id = Skyhawk.message.id
    if Skyhawk.reply_to_msg_id:
        reply_to_id = Skyhawk.reply_to_msg_id
    event = await edit_or_reply(Skyhawk, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await Skyhawk.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await Skyhawk.client.send_file(
                event.chat_id,
...
async def _(event):
    if event.fwd_from:
        return
    Skyhawkreply = await event.get_reply_message()
    if not Skyhawkreply or not Skyhawkreply.media or not Skyhawkreply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if Skyhawkreply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    reply_to_id = event.message
    if event.reply_to_msg_id:
        reply_to_id = await event.get_reply_message()
    chat = "@tgstogifbot"
    Skyhawkevent = await edit_or_reply(event, "`Converting to gif ...`")
    async with event.client.conversation(chat) as conv:
        try:
            await silently_send_message(conv, "/start")
            await event.client.send_file(chat, Skyhawkreply.media)
            response = await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            if response.text.startswith("Send me an animated sticker!"):
                return await Skyhawkevent.edit("`This file is not supported`")
            Skyhawkresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            Skyhawkfile = Path(await event.client.download_media(Skyhawkresponse, "./temp/"))
            Skyhawkgif = Path(await unzip(Skyhawkfile))
            ...
            for files in (Skyhawkgif, Skyhawkfile):
                if files and os.path.exists(files):
                    os.remove(files)
        except YouBlockedUserError:
            await Skyhawkevent.edit("Unblock @tgstogifbot")
            return
