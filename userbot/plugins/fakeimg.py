...
from SkyhawkBot.utils import edit_or_reply, admin_cmd, sudo_cmd
import os
...
async def _(event):
    ...
    if response.status_code == 200:
      with open("SkyhawkBot.jpg", 'wb') as f:
        f.write(response.content)
    
    captin = f"Fake Image By SkyhawkBot."
    fole = "SkyhawkBot.jpg"
    await borg.send_file(event.chat_id, fole, caption=captin)
    await event.delete()
    os.system("rm /root/userbot/SkyhawkBot.jpg ")