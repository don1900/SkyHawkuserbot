...
Skyhawk_logo = "./Skyhawk/Skyhawk_bot_logo.jpg"
furious = SkyhawkBot.uid
Skyhawk = f"[{DEFAULTUSER}](tg://user?id={furious})"

@SkyhawkBot.on(admin_cmd(pattern=r"send (?P<shortname>\w+)", outgoing=True))
@SkyhawkBot.on(sudo_cmd(pattern=r"send (?P<shortname>\w+)", allow_sudo=True))
async def send(event):
    ...
    omk = f"**⍟ Plugin name ≈** `{input_str}`\n**⍟ Uploaded by ≈** {Skyhawk}\n\n⚡ **[Legendary SkyhawkBot](t.me/SkyHawK_Updates)** ⚡"
...
@SkyhawkBot.on(admin_cmd(pattern="install$", outgoing=True))
@SkyhawkBot.on(sudo_cmd(pattern="install$", allow_sudo=True))
                ...
                if shortname in CMD_LIST:
                    ...
                    for i in CMD_LIST[shortname]:
                        ...
                        if b == 1:
                            ...
                            b = 1
                        await event.edit(a)
                    return await event.edit(f"✅ **Installed module** :- `{shortname}` \n✨ BY :- {Skyhawk}\n\n{string}\n\n        ⚡ **[Legendary SkyhawkBot](t.me/SkyHawK_Updates)** ⚡", link_preview=False)
...
@SkyhawkBot.on(admin_cmd(pattern=r"uninstall (?P<shortname>\w+)", outgoing=True))
@SkyhawkBot.on(sudo_cmd(pattern=r"uninstall (?P<shortname>\w+)", allow_sudo=True))
...
@SkyhawkBot.on(admin_cmd(pattern=r"unload (?P<shortname>\w+)$"))
@SkyhawkBot.on(sudo_cmd(pattern=r"upload (?P<shortname>\w+)$", allow_sudo=True))
...
@SkyhawkBot.on(admin_cmd(pattern=r"load (?P<shortname>\w+)$"))
@SkyhawkBot.on(sudo_cmd(pattern=r"load (?P<shortname>\w+)$", allow_sudo=True))
...
CmdHelp("core").add_command(
  ...
  "cmds", None, "Gives out the list of modules in SkyhawkBot."
).add()
